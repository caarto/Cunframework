...
public class WildcardOfObject {
	private static Logger logger = LoggerFactory.getLogger(WildcardOfObject.class);
	...
	public static String format(Object[] obj ,String input, String regex){
		...
		while (m.find()) { // 遍历匹配项
			...
			if(num>obj.length-1){
				logger.error("下标越界错误,查看'.'前的数字(起始位置为0)是否超过了传入参数的个数! 参数个数:"+obj.length+",位置:"+num);
				return "";
			}
