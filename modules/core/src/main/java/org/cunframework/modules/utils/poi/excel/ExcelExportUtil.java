package org.cunframework.modules.utils.poi.excel;
...
import org.cunframework.modules.utils.poi.excel.annotation.Excel;
import org.cunframework.modules.utils.poi.excel.annotation.ExcelCollection;
import org.cunframework.modules.utils.poi.excel.annotation.ExcelTarget;
import org.cunframework.modules.utils.poi.excel.entity.ComparatorExcelField;
import org.cunframework.modules.utils.poi.excel.entity.ExcelExportEntity;
import org.cunframework.modules.utils.poi.excel.entity.ExcelTitle;
import org.cunframework.modules.utils.poi.excel.entity.TemplateExportParams;
...
public final class ExcelExportUtil {
	 ...
	 * @return
	 */
	public static HSSFWorkbook exportExcel(HttpServletRequest request, List<Map<String, Object>> list) {
		HSSFWorkbook workbook = new HSSFWorkbook();
		for (Map<String, Object> map : list) {
			createSheetInUserModel2File(request,workbook,
	 ...
	 *            输出流
	 */
	public static HSSFWorkbook exportExcel(HttpServletRequest request,
			ExcelTitle entity,
			Class<?> pojoClass, Collection<?> dataSet) {
		HSSFWorkbook workbook = new HSSFWorkbook();
		createSheetInUserModel2File(request, workbook, entity, pojoClass, dataSet);
		return workbook;
	}
	...
	private static void createSheetInUserModel2File(HttpServletRequest request, 
		...
		try {
			...
			while (its.hasNext()) {
				Object t = its.next();
				index += createCells(request, patriarch,index, t, excelParams, sheet, workbook
						,styles);
			}
		} catch (Exception e) {
			e.printStackTrace();
	 ...
	 * @throws Exception
	 */
	private static int createCells(HttpServletRequest request, Drawing patriarch, int index, Object t,
		...
		for (int k = 0, paramSize = excelParams.size(); k < paramSize; k++) {
			...
			if (entity.getList() != null) {
				...
				int listC = 0;
				for (Object obj : list) {
					createListCells(request, patriarch,index + listC, cellNum, obj,
				...
				if (entity.getType() == 1) {
					...
					createImageCell(request, patriarch,entity, row, cellNum++, value == null ? ""
							: value.toString(),t);
				}
			}
		}
	 ...
	 * @param styles 
	 */
	private static void createListCells(HttpServletRequest request, Drawing patriarch, int index, int cellNum, Object obj,
			List<ExcelExportEntity> excelParams, Sheet sheet,
			HSSFWorkbook workbook, Map<String, HSSFCellStyle> styles) throws Exception {
		ExcelExportEntity entity;
		Row row;
	 ...
	 * @throws Exception 
	 */
	private static void createImageCell(HttpServletRequest request, Drawing patriarch, ExcelExportEntity entity, Row row,
